...
export class IdentityService {
    ...
    requireViewerSession(viewerId: number): Session {
        const session = this.getUsableSession(String(viewerId));
        if (session === null || session.type !== SessionType.VIEWER) {
            throw new InvalidCredentialsError("Invalid viewer id.");
        }
        return session;
    }

    requireViewer(viewerId: number, accountId: number): Session {
        const session = this.requireViewerSession(viewerId);
